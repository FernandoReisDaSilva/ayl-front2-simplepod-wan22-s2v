...
TEST_ID = "TEST_RUNPOD_LATENTSYNC_IMAGE_V1_1_LATENTSYNC_PROBE_0001"
REPO_ROOT = Path(__file__).resolve().parents[2]
ENV_PATH = REPO_ROOT / ".env"
GRAPHQL_ENDPOINT = "https://api.runpod.io/graphql"
OUTPUT_DIR = REPO_ROOT / "tmp" / "runpod_latentsync_image_v1_1_latentsync_probe_0001"
INTENDED_PAYLOAD_PATH = OUTPUT_DIR / "intended_payload.json"
LOCAL_FINAL_REPORT_PATH = OUTPUT_DIR / "output" / "final_report.json"
LOG_PATH = REPO_ROOT / "logs" / "runpod_latentsync_image_v1_1_latentsync_probe_0001_log.json"
...
def intended_payload(args: argparse.Namespace, marker_nonce: str) -> dict:
    return {
        "test_id": TEST_ID,
        "purpose": "validate_latentsync_image_v1_1_latentsync_probe_without_dockerargs",
        "image_tag": args.image_tag,
        "run_mode": "latentsync_probe",
        ...
        "no_inference": True,
        "technical_success_does_not_require_torch_or_cuda_success": True,
        "required_final_report_fields": list(REQUIRED_FINAL_REPORT_FIELDS),
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if not execute_allowed:
            ...
            print("RunPod LatentSync image V1.1 LatentSync environment probe dry-run created. No mutation sent and no Pod created.")
            print(f"Intended payload written: {INTENDED_PAYLOAD_PATH}")
            print(f"Log written: {LOG_PATH}")
            return 0

        trace("START execute mode")
        trace(f"CONFIG image={args.image_tag} gpu_type_id={args.gpu_type_id} timeout={args.max_wait_seconds}s run_mode=latentsync_probe")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Dry-run or execute the LatentSync image V1.1 LatentSync environment probe.")
