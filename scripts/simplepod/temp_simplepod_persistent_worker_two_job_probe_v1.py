...
SCRIPT_ID = "TEMP_SIMPLEPOD_PERSISTENT_WORKER_TWO_JOB_PROBE_V1"
REPO_ROOT = Path(__file__).resolve().parents[2]
REPORT_PATH = REPO_ROOT / "logs" / "simplepod_persistent_worker_two_job_probe_v1.json"
...
def runtime_payload(instance_market: str) -> dict:
    payload = {
        ...
        "envVariables": [
            ...
            {"name": "AYL_RUNTIME_VERSION", "value": "v2-blackwell-persistent-two-job-probe-v1"},
...
def summarize_worker(value) -> dict:
    ...
    keys = (
        ...
        "jobs_completed",
        "last_load_seconds",
        "resident_vram_allocated_gb",
        "resident_vram_reserved_gb",
        "resident_model_objects",
        "t5_cpu_effective",
        "offload_model_effective",
        "convert_model_dtype_effective",
        "last_recycle_decision",
    )
    return {key: value.get(key) for key in keys if key in value}


def job_allows_next(job_result: dict, status_json: dict) -> tuple[bool, list[str]]:
    reasons = []
    ...
    if job_result.get("worker_state_after_job") != "ready":
        reasons.append("worker_not_ready_after_job")
    ...
    free_margin = job_result.get("free_margin_after_cleanup_gb")
    try:
        if free_margin is not None and float(free_margin) < 8.0:
            reasons.append("free_margin_below_8gb")
    except (TypeError, ValueError):
        pass
    if isinstance(status_json, dict) and status_json.get("worker_state") != "ready":
        reasons.append("status_worker_not_ready")
...
def build_report(args: argparse.Namespace, status: str, data: dict, error: str = "") -> dict:
    ...
    return {
        ...
        "jobs": {
            "job1_payload": job_payload(1),
            "job2_payload": job_payload(2),
            "job1_result": data.get("job1_result", {}),
            "job2_result": data.get("job2_result", {}),
            "job1_status_after": data.get("job1_status_after", {}),
            "job2_status_after": data.get("job2_status_after", {}),
            "job1_next_gate": data.get("job1_next_gate", {}),
        },
        "downloads": {
            "job1_video": data.get("download_job1_video", r2_transfer_file(local_output_paths(1)["video"], job_payload(1)["output_video_key"], upload=False, dry_run=True)),
            "job1_report": data.get("download_job1_report", r2_transfer_file(local_output_paths(1)["report"], job_payload(1)["output_report_key"], upload=False, dry_run=True)),
            "job2_video": data.get("download_job2_video", r2_transfer_file(local_output_paths(2)["video"], job_payload(2)["output_video_key"], upload=False, dry_run=True)),
            "job2_report": data.get("download_job2_report", r2_transfer_file(local_output_paths(2)["report"], job_payload(2)["output_report_key"], upload=False, dry_run=True)),
        ...
        "success_conditions": {
            ...
            "job2_aborts_on_oom_recycle_low_margin_hook_pipeline_or_reload": True,
...
def run(args: argparse.Namespace) -> int:
    data = {"delete_result": {"attempted": False, "status": "not_started"}}
    timer = PhaseTimer()
    ...
    try:
        dry_run = not execute_allowed(args)
        print(f"[{SCRIPT_ID}] START dry_run={str(dry_run).lower()} image={IMAGE_REF}", flush=True)
        ...
        with timer.phase("persistent_worker_run_job1"):
            job1 = base.simple_post(proxy_url + RUN_JOB_ENDPOINT, job_payload(1), timeout_seconds=args.job_timeout_seconds)
        data["job1_result"] = compact_http_result(job1)
        job1_json = job1.get("json") if isinstance(job1.get("json"), dict) else {}
        if job1.get("http_status_code") != 200 or job1_json.get("status") != "succeeded":
            status = "job1_failed"
            return 1

        with timer.phase("persistent_worker_status_after_job1"):
            job1_status = base.simple_get(proxy_url + STATUS_ENDPOINT, timeout_seconds=30)
        data["job1_status_after"] = compact_http_result(job1_status)
        can_continue, reasons = job_allows_next(job1_json, job1_status.get("json") if isinstance(job1_status.get("json"), dict) else {})
        data["job1_next_gate"] = {"can_continue": can_continue, "reasons": reasons}
        if not can_continue:
            status = "job2_aborted_after_job1_safety_gate"
            return 1

        with timer.phase("persistent_worker_run_job2"):
            job2 = base.simple_post(proxy_url + RUN_JOB_ENDPOINT, job_payload(2), timeout_seconds=args.job_timeout_seconds)
        data["job2_result"] = compact_http_result(job2)
        job2_json = job2.get("json") if isinstance(job2.get("json"), dict) else {}
        if job2.get("http_status_code") != 200 or job2_json.get("status") != "succeeded":
            status = "job2_failed"
            return 1

        with timer.phase("persistent_worker_status_after_job2"):
            job2_status = base.simple_get(proxy_url + STATUS_ENDPOINT, timeout_seconds=30)
        data["job2_status_after"] = compact_http_result(job2_status)

        with timer.phase("download_outputs_from_r2"):
            data["download_job1_video"] = r2_transfer_file(local_output_paths(1)["video"], job_payload(1)["output_video_key"], upload=False, dry_run=False)
            data["download_job1_report"] = r2_transfer_file(local_output_paths(1)["report"], job_payload(1)["output_report_key"], upload=False, dry_run=False)
            data["download_job2_video"] = r2_transfer_file(local_output_paths(2)["video"], job_payload(2)["output_video_key"], upload=False, dry_run=False)
            data["download_job2_report"] = r2_transfer_file(local_output_paths(2)["report"], job_payload(2)["output_report_key"], upload=False, dry_run=False)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a two-job persistent Wan2.2 S2V worker probe on one SimplePod.")
    parser.add_argument("--execute", action="store_true", help="Create a real SimplePod instance and run two real jobs.")
