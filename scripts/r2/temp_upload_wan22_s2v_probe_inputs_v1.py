...
TEST_ID = "TEST_R2_WAN22_S2V_PROBE_INPUTS_UPLOAD_V1"
REPO_ROOT = Path(__file__).resolve().parents[2]
ENV_PATH = REPO_ROOT / ".env"
LOG_PATH = REPO_ROOT / "logs" / "r2_wan22_s2v_probe_inputs_upload_v1_log.json"
...
UPLOAD_TARGETS = (
    ("reference_image", "reference_image_local", "tests/runpod_wan22_s2v_probe_v1/input/mae_reference.png"),
    ("audio", "audio_local", "tests/runpod_wan22_s2v_probe_v1/input/mae_audio_5s.wav"),
)
TARGET_GROUPS = {
    "reference": {"reference_image"},
    "audio": {"audio"},
    "inputs": {"reference_image", "audio"},
}
...
def selected_scope(args: argparse.Namespace) -> str:
    selected = [name for name, enabled in (("reference", args.only_reference), ("audio", args.only_audio), ("inputs", args.only_inputs)) if enabled]
    if len(selected) > 1:
        ...
        items.append(
            {
                ...
                "local_size_bytes": path.stat().st_size if is_file else 0,
                "local_sha256": sha256_file(path) if is_file else "",
                "r2_key": r2_key,
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        for item in items:
            print(f"[{TEST_ID}] LOCAL {'ok' if item['local_is_file'] else 'missing'} size={item['local_size_bytes']} key={item['r2_key']}")
        if problems:
        ...
        for item in items:
            ...
            item["remote_after"] = head_object(client, config["bucket"], item["r2_key"])
            item["upload_status"] = "uploaded" if item["remote_after"].get("exists") else "head_after_upload_missing"
            print(f"[{TEST_ID}] UPLOADED size={item['local_size_bytes']} key={item['r2_key']}")
        status = "succeeded" if not upload_errors and all(item["upload_status"] == "uploaded" for item in items) else "upload_failed"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Dry-run or upload Wan2.2 S2V probe inputs to R2.")
    parser.add_argument("--reference-image-local", default="data/wan27/inputs/mae_reference.png")
    parser.add_argument("--audio-local", default="~/Downloads/mae_audio_5s.wav")
