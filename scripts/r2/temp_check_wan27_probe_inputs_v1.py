...
TEST_ID = "TEST_R2_WAN27_PROBE_INPUTS_CHECK_V1"
REPO_ROOT = Path(__file__).resolve().parents[2]
ENV_PATH = REPO_ROOT / ".env"
LOG_PATH = REPO_ROOT / "logs" / "r2_wan27_probe_inputs_check_v1_log.json"
    ...
    try:
        if dry_run:
            ...
            for key in OBJECT_KEYS:
                item = {"key": key, "exists": False, "check_status": "planned", "size_bytes": 0, "error": ""}
                results.append(item)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Dry-run or HEAD-check required WAN 2.7 probe inputs in R2.")
